...
async function handleApi(request, env, url) {
  ...
  if (pathname === "/api/sales/finance" && method === "GET") {
    ...
    const agentQ = isAdmin ? "sales_agents?select=*&order=name.asc&limit=1000" : "sales_agents?deleted=eq.false&id=eq." + encodeURIComponent(agentId) + "&select=*&limit=1";
    const scope = isAdmin ? "" : "deleted=eq.false&agent_id=eq." + encodeURIComponent(agentId) + "&";
  ...
  if (pathname === "/api/upload" && method === "POST") {
    ...
    const maxBytes = kind === "product" ? 20 * 1024 * 1024 : kind === "menu" ? 15 * 1024 * 1024 : kind === "video" ? 8 * 1024 * 1024 : 3 * 1024 * 1024;
    if (file.size > maxBytes) {
      const limitLabel = kind === "product" ? "20 MB" : kind === "menu" ? "15 MB" : kind === "video" ? "8 MB" : "3 MB";
