...
async function ensurePaidSaleCashRows(env, orderRow) {
  ...
  const at = orderRow.sold_at || (/* @__PURE__ */ new Date()).toISOString();
  if (!cats.has("sale") && Number(orderRow.amount) > 0) {
    await upsertSalesRow(env, "sales_cashflow", {
      ...
      description: `Sale \xB7 ${orderRow.product_name || "Product"} \xB7 ${orderRow.customer_name || ""}`.trim(),
      sale_id: orderRow.id,
  ...
  if (!cats.has("commission") && Number(orderRow.commission) > 0 && orderRow.agent_id) {
    await upsertSalesRow(env, "sales_cashflow", {
      ...
      method: "eft",
      description: `Commission \xB7 ${orderRow.product_name || "Sale"}`.trim(),
      sale_id: orderRow.id,
...
__name(ensurePaidSaleCashRows, "ensurePaidSaleCashRows");
...
__name(preferredShareSlug, "preferredShareSlug");
...
async function provisionTeamFromShopQuote(env, {
  ...
  const teamName = String(company || name || "Connect Team").trim().slice(0, 120) || "Connect Team";
  const id = uid("team");
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function handleApi(request, env, url) {
  ...
  if (pathname === "/api/auth/signup" && method === "POST") {
    ...
    const pendingTeamInvite = claimCard ? await findPendingTeamInviteForCard(env, claimCard.id, id) : null;
  ...
  if (pathname === "/api/upload" && method === "POST") {
    ...
    const maxBytes = kind === "product" ? 20 * 1024 * 1024 : kind === "menu" || kind === "catalog" ? 15 * 1024 * 1024 : kind === "video" ? 8 * 1024 * 1024 : 3 * 1024 * 1024;
  ...
  if (pathname === "/api/meetings" && method === "POST") {
    ...
    try {
      ...
      const ownerEmail = String(owner?.login_email || owner?.email || "").trim().toLowerCase();
      const whenLabel = new Date(preferredAt).toUTCString();
      const meetingTitle = `Meeting with ${ownerName}`;
      const meetingDesc = [
        message ? `Message: ${message}` : "",
        phone ? `Guest phone: ${phone}` : "",
        `Booked via tap-na \xB7 https://tapnam.com/meetings`
      ].filter(Boolean).join("\n");
      ...
      sends.push(
        sendCloudflareEmail(env, {
          to: email,
          replyTo: ownerEmail.includes("@") ? ownerEmail : void 0,
