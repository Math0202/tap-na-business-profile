...
// worker/index.js
var SLUG_ALPHABET = "23456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
var TABLE_ORIGIN = "https://tapnam.com";
var CANONICAL_ORIGIN = "https://tapnam.com";
var CORS_HEADERS = {
...
__name(publicOriginForKind, "publicOriginForKind");
...
__name(cardPageUrl, "cardPageUrl");
...
__name(recordCardActivity, "recordCardActivity");
function parseUa(ua = "") {
  const s = String(ua);
  let device = "desktop";
...
__name(sanitizeErrorContext, "sanitizeErrorContext");
async function logAppError(env, {
  ...
  const safeMessage = String(message || "Unknown error").slice(0, 4e3);
  const safeStack = String(stack || "").slice(0, 8e3);
...
__name(logAppError, "logAppError");
...
__name(mapErrorLogRow, "mapErrorLogRow");
...
__name(ensureProfileStub, "ensureProfileStub");
async function getSessionProfile(env, request) {
  ...
  const sessions = await sb(
    env,
    `sessions?token=eq.${encodeURIComponent(token)}&expires_at=gt.${encodeURIComponent((/* @__PURE__ */ new Date()).toISOString())}&select=profile_id`
  );
  if (!sessions?.length) return null;
  const profiles = await sb(env, `profiles?id=eq.${encodeURIComponent(sessions[0].profile_id)}&select=*`);
  return profiles?.[0] || null;
}
__name(getSessionProfile, "getSessionProfile");
var DEFAULT_STAFF_ADMIN_EMAIL = "admin@01";
var DEFAULT_STAFF_ADMIN_PASSWORD = "Math@@0202";
...
__name(normalizeStaffRole, "normalizeStaffRole");
function isSalesElevated(staff) {
  return staff?.role === "admin" || staff?.role === "manager";
}
__name(isSalesElevated, "isSalesElevated");
...
__name(staffClaimsFromUser, "staffClaimsFromUser");
async function authAdminFetch(env, path, { method = "GET", body } = {}) {
  ...
  if (!res.ok) {
    const msg = data?.msg || data?.error_description || data?.message || data?.error || text || `HTTP ${res.status}`;
    const err = new Error(msg);
    err.status = res.status;
    err.data = data;
    await logAppError(env, {
...
__name(authAdminFetch, "authAdminFetch");
...
__name(ensureDefaultStaffAdmin, "ensureDefaultStaffAdmin");
...
__name(getStaffFromRequest, "getStaffFromRequest");
...
__name(staffSessionPayload, "staffSessionPayload");
function mapSalesProductRow(row) {
  const images = Array.isArray(row?.images) ? row.images.filter((src) => typeof src === "string" && src.trim()) : [];
...
__name(mapSalesProductRow, "mapSalesProductRow");
...
__name(mapSalesProductPublic, "mapSalesProductPublic");
function salesProductToDb(body, { isNew = false } = {}) {
  const categoryRaw = String(body?.category || "").trim();
  const category = categoryRaw === "table" || categoryRaw === "personal" || categoryRaw === "other" ? categoryRaw : body?.section === "table-brochure" ? "table" : "personal";
  const images = Array.isArray(body?.images) ? body.images.filter((src) => typeof src === "string" && src.trim()).map((src) => src.trim()) : body?.image ? [String(body.image).trim()] : [];
  return {
    ...
    ...isNew ? { created_at: (/* @__PURE__ */ new Date()).toISOString() } : {},
    updated_at: (/* @__PURE__ */ new Date()).toISOString()
  };
}
__name(salesProductToDb, "salesProductToDb");
...
__name(mapSalesAgentRow, "mapSalesAgentRow");
function salesAgentToDb(body, { isNew = false } = {}) {
  ...
  return {
    ...
    updated_at: (/* @__PURE__ */ new Date()).toISOString(),
    ...isNew ? { created_at: body?.createdAt || (/* @__PURE__ */ new Date()).toISOString() } : {}
  };
}
__name(salesAgentToDb, "salesAgentToDb");
...
__name(mapSalesOrderRow, "mapSalesOrderRow");
function salesOrderToDb(body, { isNew = false } = {}) {
  ...
  return {
    ...
    sold_at: body?.soldAt || (/* @__PURE__ */ new Date()).toISOString(),
    notes: String(body?.notes || "").trim(),
...
__name(salesOrderToDb, "salesOrderToDb");
...
__name(mapSalesQuoteRow, "mapSalesQuoteRow");
...
__name(salesQuoteToDb, "salesQuoteToDb");
...
__name(mapSalesInvoiceRow, "mapSalesInvoiceRow");
function salesInvoiceToDb(body, { isNew = false } = {}) {
  ...
  return {
    ...
    status,
    payment_method: String(body?.paymentMethod || "eft"),
    issued_at: body?.issuedAt || (/* @__PURE__ */ new Date()).toISOString(),
...
__name(salesInvoiceToDb, "salesInvoiceToDb");
...
__name(mapSalesCashRow, "mapSalesCashRow");
function salesCashToDb(body, { isNew = false } = {}) {
  ...
  return {
    ...
    sale_id: saleId,
    agent_id: agentId,
    occurred_at: body?.at || (/* @__PURE__ */ new Date()).toISOString(),
    ...isNew ? { created_at: body?.createdAt || (/* @__PURE__ */ new Date()).toISOString() } : {}
  };
}
__name(salesCashToDb, "salesCashToDb");
...
__name(upsertSalesRow, "upsertSalesRow");
...
__name(ensureSalesAgentExists, "ensureSalesAgentExists");
...
__name(ensureSalesProductOrNull, "ensureSalesProductOrNull");
...
__name(assertAgentAccess, "assertAgentAccess");
...
__name(withDeletedFields, "withDeletedFields");
...
__name(writeSalesChangeLog, "writeSalesChangeLog");
...
__name(softDeleteSalesEntity, "softDeleteSalesEntity");
...
__name(restoreSalesEntity, "restoreSalesEntity");
var DELETED_SALES_TABLES = [
  ["sales_cashflow", "cash"],
...
__name(purgeDeletedSalesRows, "purgeDeletedSalesRows");
async function softDeleteFinanceForDoomedAgents(env) {
  ...
  for (const agentId of ids) {
    for (const table of ["sales_orders", "sales_quotes", "sales_invoices", "sales_cashflow"]) {
      const body = table === "sales_cashflow" ? { deleted: true, deleted_at: now, deleted_by: "purge:agent-cascade" } : { ...patch, updated_at: now };
...
__name(softDeleteFinanceForDoomedAgents, "softDeleteFinanceForDoomedAgents");
async function softDeleteRow(env, {
  ...
  if (table !== "sales_cashflow" && table !== "card_opens" && table !== "sessions") {
    if (extra.updated_at === void 0 && table !== "cards") {
    }
  }
...
__name(softDeleteRow, "softDeleteRow");
...
__name(hardDeleteProfile, "hardDeleteProfile");
...
__name(mapChangeLogRow, "mapChangeLogRow");
...
__name(mapPublicCardRow, "mapPublicCardRow");
async function publicProfile(env, row, { includeCards = false } = {}) {
  if (!row) return null;
  const cardRows = await sb(
    env,
...
function normalizeCatalogAttachments(item) {
  const images = (Array.isArray(item?.images) ? item.images : []).map((u) => String(u || "").trim()).filter(isHttpUrl).slice(0, 8);
  const pdfs = (Array.isArray(item?.pdfs) ? item.pdfs : []).map((row) => {
    const url = String(row?.url || row || "").trim();
    if (!isHttpUrl(url)) return null;
    const name = String(row?.name || "Document").trim().slice(0, 120) || "Document";
    return { name, url };
  }).filter(Boolean).slice(0, 5);
  const links = (Array.isArray(item?.links) ? item.links : []).map((row) => {
...
__name(normalizeCatalogAttachments, "normalizeCatalogAttachments");
function normalizeCatalogItems(raw) {
  ...
  return raw.slice(0, 40).map((item, i) => {
    ...
    let price = null;
    if (priceRaw !== null && priceRaw !== void 0 && priceRaw !== "") {
...
__name(normalizeCatalogItems, "normalizeCatalogItems");
function normalizeCatalogCartItems(raw) {
  ...
  return raw.slice(0, 40).map((item) => {
    ...
    let price = null;
    if (item?.price !== null && item?.price !== void 0 && item?.price !== "") {
...
__name(normalizeCatalogCartItems, "normalizeCatalogCartItems");
...
__name(mapCatalogCartRow, "mapCatalogCartRow");
var PERSONAL_TYPE_RANKS = {
...
__name(assignableTeamRoles, "assignableTeamRoles");
...
__name(packageCeilingFromRoles, "packageCeilingFromRoles");
...
__name(resolveTeamPackageCeiling, "resolveTeamPackageCeiling");
...
__name(canAccessTeamFeatures, "canAccessTeamFeatures");
var SHOP_TEAM_PRODUCT_ROLES = {
  "black-card": "business",
...
__name(getProfilePersonalType, "getProfilePersonalType");
async function syncPersonalTypeAcrossDb(env, { profileId = "", cardId = "", personalType = "" } = {}) {
  ...
  if (profileId) {
    ...
    await sb(
      ...
      {
        method: "PATCH",
        body: { role, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
        prefer: "return=minimal"
      }
    );
  }
}
__name(syncPersonalTypeAcrossDb, "syncPersonalTypeAcrossDb");
...
__name(mapTeamMemberRow, "mapTeamMemberRow");
async function getOrCreateOwnedTeam(env, profile) {
  ...
  if (existing?.[0]) {
    await sb(
      ...
      {
        method: "PATCH",
        body: { role: ownerRole, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
        prefer: "return=minimal"
      }
    );
    return existing[0];
  ...
  const id = uid("team");
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const name = String(profile.company || profile.name || "My team").trim().slice(0, 120) || "My team";
  const packageCeiling = personalTypeRank(ownerRole) >= personalTypeRank("business") ? ownerRole : "business";
  const ownerEmail = String(profile.login_email || profile.email || "").trim().toLowerCase();
  await sb(env, "teams", {
    method: "POST",
    body: {
      id,
...
__name(getOrCreateOwnedTeam, "getOrCreateOwnedTeam");
...
__name(claimProvisionalTeamsForProfile, "claimProvisionalTeamsForProfile");
async function provisionTeamFromShopQuote(env, {
  ...
  const teamName = String(company || name || "Connect Teams").trim().slice(0, 120) || "Connect Teams";
  const id = uid("team");
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
__name(provisionTeamFromShopQuote, "provisionTeamFromShopQuote");
...
__name(getActorTeamMembership, "getActorTeamMembership");
async function enrichTeamMembers(env, members) {
  ...
  return (members || []).map((m) => {
    ...
    return mapTeamMemberRow({
      ...
      member_email: p ? String(p.login_email || p.email || m.invite_email || "").trim() : String(m.invite_email || "")
    });
  });
}
__name(enrichTeamMembers, "enrichTeamMembers");
...
__name(resolveSharedCatalogForProfile, "resolveSharedCatalogForProfile");
async function findPendingTeamInviteForCard(env, cardId, profileId = "") {
  let q = `team_members?deleted=eq.false&status=in.(pending_claim,invited)&select=*&order=created_at.desc&limit=5`;
...
__name(findPendingTeamInviteForCard, "findPendingTeamInviteForCard");
async function sendTeamInviteEmail(env, {
  ...
  const roleLabel = role === "executive_exclusive" ? "Executive Exclusive" : role === "business" ? "Business" : "Professional";
  const subject = pendingClaim ? `You've been invited to join ${teamName} on tap-na` : `${ownerName} invited you to ${teamName} on tap-na`;
  const bodyHtml = pendingClaim ? `<p style="margin:0 0 8px;">When you claim card <strong>${escapeHtml(slug)}</strong>, you'll be asked to join <strong>${escapeHtml(teamName)}</strong> as <strong>${escapeHtml(roleLabel)}</strong>.</p>
       <p style="margin:12px 0 0;">Open your card link and complete signup to accept or decline.</p>` : `<p style="margin:0 0 8px;"><strong>${escapeHtml(ownerName)}</strong> invited you to join <strong>${escapeHtml(teamName)}</strong> as <strong>${escapeHtml(roleLabel)}</strong>.</p>
  ...
  await sendCloudflareEmail(env, {
    ...
    text: [
      subject,
      pendingClaim ? `Claim card ${slug} to join ${teamName} as ${roleLabel}.` : `Open https://tapnam.com/team to respond.`
    ].join("\n")
  });
}
__name(sendTeamInviteEmail, "sendTeamInviteEmail");
...
__name(defaultEmailFrom, "defaultEmailFrom");
function defaultEmailReplyTo(env) {
  return String(env.EMAIL_REPLY_TO || "").trim();
}
__name(defaultEmailReplyTo, "defaultEmailReplyTo");
...
__name(pdfEscapeText, "pdfEscapeText");
function buildSimplePdfBase64(title, lines = []) {
  const allLines = [String(title || "Document").slice(0, 80), "", ...Array.isArray(lines) ? lines : []].map((l) => pdfEscapeText(l).slice(0, 95)).slice(0, 45);
...
__name(buildSimplePdfBase64, "buildSimplePdfBase64");
...
__name(formatIcsUtc, "formatIcsUtc");
function buildMeetingIcs({
  ...
  const end = new Date(start.getTime() + Math.max(15, Number(durationMinutes) || 30) * 60 * 1e3);
  const stamp = formatIcsUtc(/* @__PURE__ */ new Date());
  ...
  const lines = [
    ...
    orgMail ? `ORGANIZER;CN=${icsEscape(organizerName || orgMail)}:mailto:${orgMail}` : "",
    orgMail ? `ATTENDEE;CN=${icsEscape(organizerName || orgMail)};ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;RSVP=FALSE:mailto:${orgMail}` : "",
    attMail ? `ATTENDEE;CN=${icsEscape(attendeeName || attMail)};ROLE=REQ-PARTICIPANT;PARTSTAT=NEEDS-ACTION;RSVP=TRUE:mailto:${attMail}` : "",
    "STATUS:CONFIRMED",
    "SEQUENCE:0",
    "TRANSP:OPAQUE",
    "END:VEVENT",
...
__name(buildMeetingIcs, "buildMeetingIcs");
...
__name(meetingInviteAttachment, "meetingInviteAttachment");
function meetingWindow(startIso, durationMinutes = 30) {
  const start = new Date(startIso);
  if (Number.isNaN(start.getTime())) throw new Error("Invalid meeting start time");
  const minutes = Math.max(15, Number(durationMinutes) || 30);
  const end = new Date(start.getTime() + minutes * 60 * 1e3);
  return { start, end, minutes };
}
__name(meetingWindow, "meetingWindow");
...
__name(googleCalendarUrl, "googleCalendarUrl");
var OUTLOOK_CALENDAR_URL = "https://outlook.cloud.microsoft/calendar/view/workweek?deeplink=mail%2F";
function outlookCalendarUrl() {
  return OUTLOOK_CALENDAR_URL;
}
__name(outlookCalendarUrl, "outlookCalendarUrl");
...
__name(meetingInviteToken, "meetingInviteToken");
...
__name(meetingInviteIcsUrl, "meetingInviteIcsUrl");
...
__name(ownerNotifyEmails, "ownerNotifyEmails");
var CRM_APP_URLS = {
...
function parseTeamIntegrationsFromBody(body) {
  const meetingTool = String(body?.meetingTool || body?.meeting_tool || "").trim().toLowerCase();
  const usesCrm = !!(body?.usesCrm ?? body?.uses_crm);
  const crmProvider = String(body?.crmProvider || body?.crm_provider || "").trim().toLowerCase();
...
__name(parseTeamIntegrationsFromBody, "parseTeamIntegrationsFromBody");
...
__name(applyTeamIntegrations, "applyTeamIntegrations");
...
__name(teamIntegrationsForProfile, "teamIntegrationsForProfile");
function calendarButtonRow(href, logoSrc, label) {
  const img = logoSrc ? `<img src="${escapeHtml(logoSrc)}" width="22" height="22" alt="" style="display:inline-block;width:22px;height:22px;vertical-align:middle;margin-right:10px;border:0;" />` : "";
...
__name(calendarButtonRow, "calendarButtonRow");
...
__name(calendarAddLinksHtml, "calendarAddLinksHtml");
...
__name(calendarAddLinksText, "calendarAddLinksText");
function crmAddLinksHtml({ usesCrm, crmProvider, crmOther, vcfUrl }) {
  if (!usesCrm || !crmProvider) return "";
  const label = crmProvider === "other" ? String(crmOther || "Others").trim() || "Others" : CRM_LABELS[crmProvider] || "CRM";
...
__name(crmAddLinksHtml, "crmAddLinksHtml");
...
__name(crmAddLinksText, "crmAddLinksText");
...
__name(contactAddLinkHtml, "contactAddLinkHtml");
function contactAddLinkText(vcfUrl) {
  if (!vcfUrl) return "";
  return ["Guest contact", `Add contact: ${vcfUrl}`].join("\n");
}
__name(contactAddLinkText, "contactAddLinkText");
function buildGuestVcard({ name, email, phone, note }) {
  const esc = /* @__PURE__ */ __name((v) => String(v || "").replace(/\\/g, "\\\\").replace(/,/g, "\\,").replace(/;/g, "\\;").replace(/\n/g, "\\n"), "esc");
  return [
    "BEGIN:VCARD",
    "VERSION:3.0",
    `FN:${esc(name)}`,
...
__name(buildGuestVcard, "buildGuestVcard");
...
__name(meetingContactVcfUrl, "meetingContactVcfUrl");
function meetingInvitePayload({
  ...
  const description = [
    ...
    `Booked via tap-na \xB7 ${CANONICAL_ORIGIN}/meetings`
  ].filter(Boolean).join("\n");
  return {
    ownerName,
    ownerEmail,
...
__name(meetingInvitePayload, "meetingInvitePayload");
...
__name(normalizeAttachmentList, "normalizeAttachmentList");
async function sendCloudflareEmail(env, opts = {}) {
  const toList = Array.isArray(opts.to) ? opts.to.map((x) => String(x || "").trim()).filter(Boolean) : String(opts.to || "").split(",").map((x) => x.trim()).filter(Boolean);
  ...
  if (!res.ok || data?.success === false) {
    const err = data?.errors?.[0]?.message || data?.messages?.[0]?.message || data?.error || `Cloudflare Email HTTP ${res.status}`;
...
__name(sendCloudflareEmail, "sendCloudflareEmail");
...
__name(transactionalShell, "transactionalShell");
...
async function sendLoginAlertEmail(env, { email, name }) {
  const display = String(name || "").trim() || "there";
  const when = (/* @__PURE__ */ new Date()).toUTCString();
  const html = transactionalShell({
    title: "New sign-in",
...
__name(sendLoginAlertEmail, "sendLoginAlertEmail");
...
__name(sendSalesAgentCredentialsEmail, "sendSalesAgentCredentialsEmail");
function ogHtml({
  title,
  description,
  url,
...
__name(absolutePublicUrl, "absolutePublicUrl");
...
__name(shopProductFallbackImage, "shopProductFallbackImage");
...
__name(loadShopProductOg, "loadShopProductOg");
async function serveOgImage(env, origin, slug) {
  ...
  const raw = (profile.card_type === "table" ? profile.logo || profile.avatar : profile.avatar || profile.logo) || "";
  if (/^https?:\/\//i.test(raw)) {
    try {
...
__name(destinationFor, "destinationFor");
async function handleApi(request, env, url) {
  ...
  if (pathname === "/api/cards/provision" && method === "POST") {
    ...
    const personalType = kind === "personal" ? normalizePersonalType(body?.personalType || body?.personal_type || "business") : "";
  ...
  if (cardMatch && method === "GET") {
    ...
    return json({
      ...
      pendingTeamInvite: card.status === "unlinked" ? await findPendingTeamInviteForCard(env, card.id) : null
    });
  }
  ...
  if (kindMatch && method === "PATCH") {
    ...
    const kind = body?.kind !== void 0 ? body.kind === "personal" ? "personal" : "table" : cardRow.kind === "personal" ? "personal" : "table";
    const patch = { kind };
    if (kind === "personal") {
  ...
  if (pathname === "/api/me/team" && method === "GET") {
    ...
    const memberQ = includeDeleted ? `team_members?team_id=eq.${encodeURIComponent(team.id)}&select=*&order=created_at.asc` : `team_members?team_id=eq.${encodeURIComponent(team.id)}&deleted=eq.false&select=*&order=created_at.asc`;
    ...
    if (normalizePersonalType(team.package_ceiling || "") !== packageCeiling) {
      await sb(env, `teams?id=eq.${encodeURIComponent(team.id)}`, {
        method: "PATCH",
        body: { package_ceiling: packageCeiling, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
        prefer: "return=minimal"
      });
      team = { ...team, package_ceiling: packageCeiling };
    }
    const members = await enrichTeamMembers(env, memberRows || []);
    const myMembership = members.find((m) => m.profileId === profile.id && !m.deleted && m.status === "active") || members.find((m) => m.profileId === profile.id && !m.deleted) || null;
    const isOwner = team.owner_profile_id === profile.id;
    const myRole = normalizePersonalType(myMembership?.role || myCardType || "business");
    const pendingInvites = (memberships || []).filter((m) => m.status === "invited" || m.status === "pending_claim").map((m) => mapTeamMemberRow(m));
    return json({
      ok: true,
  ...
  if (pathname === "/api/me/team" && method === "PUT") {
    ...
    if (body?.shareCatalog !== void 0 || body?.share_catalog !== void 0) {
      patch.share_catalog = !!(body.shareCatalog ?? body.share_catalog);
    }
    if (body?.meetingTool !== void 0 || body?.meeting_tool !== void 0 || body?.usesCrm !== void 0 || body?.uses_crm !== void 0 || body?.crmProvider !== void 0 || body?.crm_provider !== void 0 || body?.crmOther !== void 0 || body?.crm_other !== void 0) {
      const integ = parseTeamIntegrationsFromBody(body);
      const meetingSpecified = body?.meetingTool !== void 0 || body?.meeting_tool !== void 0;
      const crmSpecified = body?.usesCrm !== void 0 || body?.uses_crm !== void 0 || body?.crmProvider !== void 0 || body?.crm_provider !== void 0 || body?.crmOther !== void 0 || body?.crm_other !== void 0;
      if (meetingSpecified) {
  ...
  if (pathname === "/api/me/team/members" && method === "POST") {
    ...
    const packageCeiling = resolveTeamPackageCeiling(team, memberRows || []);
    const actorRole = isOwner ? normalizePersonalType(actorMembership?.role || packageCeiling) : normalizePersonalType(actorMembership?.role || "");
  ...
  if (teamMemberPatchMatch && method === "PATCH") {
    ...
    const rows = await sb(
      env,
      restoring ? `team_members?id=eq.${encodeURIComponent(memberId)}&select=*` : `team_members?id=eq.${encodeURIComponent(memberId)}&deleted=eq.false&select=*`
    );
    const member = rows?.[0];
    ...
    if (body?.deleted === true || body?.action === "remove") {
      ...
      await softDeleteRow(env, {
        ...
        extra: { updated_at: (/* @__PURE__ */ new Date()).toISOString() }
      });
    ...
    if (body?.role !== void 0) {
      ...
      await sb(env, `team_members?id=eq.${encodeURIComponent(memberId)}`, {
        method: "PATCH",
        body: { role: nextRole, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
        prefer: "return=minimal"
      });
      await syncPersonalTypeAcrossDb(env, {
      ...
      if (ceiling !== normalizePersonalType(team.package_ceiling || "")) {
        await sb(env, `teams?id=eq.${encodeURIComponent(team.id)}`, {
          method: "PATCH",
          body: { package_ceiling: ceiling, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
          prefer: "return=minimal"
        });
      }
  ...
  if (pathname === "/api/auth/signup" && method === "POST") {
    ...
    const pendingTeamInviteForClaim = claimCard ? await findPendingTeamInviteForCard(env, claimCard.id) : null;
    const claimPersonalType = claimCard?.kind === "personal" ? normalizePersonalType(claimCard.personal_type || "business") : "";
    const isTeamOwnerClaim = !pendingTeamInviteForClaim && (claimPersonalType === "business" || claimPersonalType === "executive_exclusive");
    ...
    const id = uid("prof");
    const cardType = claimCard ? claimCard.kind === "personal" ? "personal" : "table" : body.cardType === "table" ? "table" : "personal";
    await sb(env, "profiles", {
      method: "POST",
      body: {
        id,
    ...
    const pendingTeamInvite = claimCard ? await findPendingTeamInviteForCard(env, claimCard.id, id) : pendingTeamInviteForClaim;
  ...
  if (pathname === "/api/auth/forgot-password" && method === "POST") {
    ...
    const okMsg = "If an account exists for that email, we sent a temporary password. Log in, then change your password from Edit profile.";
    if (!id || !id.includes("@")) {
  ...
  if (pathname === "/api/sales/finance" && method === "GET") {
    ...
    const agentQ = isElevated ? "sales_agents?select=*&order=name.asc&limit=1000" : "sales_agents?deleted=eq.false&id=eq." + encodeURIComponent(agentId) + "&select=*&limit=1";
    const scope = isElevated ? "" : "deleted=eq.false&agent_id=eq." + encodeURIComponent(agentId) + "&";
  ...
  if (pathname === "/api/sales/changelog" && method === "GET") {
    const gate = await requireStaff(env, request, { roles: ["admin", "manager"] });
    if (gate.error) return gate.error;
    const limit = Math.min(1e3, Math.max(1, Number(url.searchParams.get("limit") || 200) || 200));
    const rows = await sb(
      env,
  ...
  if (pathname === "/api/client-errors" && method === "POST") {
    ...
    await logAppError(env, {
      source: "client",
      message: String(body?.message || "Client error").slice(0, 4e3),
      stack: String(body?.stack || "").slice(0, 8e3),
  ...
  if (pathname === "/api/sales/agents" && method === "PUT") {
    ...
    const existing = existingId ? await sb(env, "sales_agents?id=eq." + encodeURIComponent(existingId) + "&select=*") : [];
    const beforeRow = existing?.[0] || null;
    ...
    const payload = {
      ...row,
      created_at: beforeRow?.created_at || body?.createdAt || row.created_at || (/* @__PURE__ */ new Date()).toISOString()
    };
  ...
  if (salesAgentMatch && method === "DELETE") {
    ...
    const now = (/* @__PURE__ */ new Date()).toISOString();
    for (const table of ["sales_orders", "sales_quotes", "sales_invoices", "sales_cashflow"]) {
      const body = table === "sales_cashflow" ? { deleted: true, deleted_at: now, deleted_by: gate.staff?.email || gate.staff?.id || "agent-delete" } : {
        deleted: true,
        deleted_at: now,
  ...
  if (pathname === "/api/sales/orders" && method === "PUT") {
    ...
    const body = await readJson(request);
    const existingId = String(body?.id || "").trim();
    const existing = existingId ? await sb(env, "sales_orders?id=eq." + encodeURIComponent(existingId) + "&select=*") : [];
    const beforeRow = existing?.[0] || null;
  ...
  if (pathname === "/api/sales/quotes" && method === "PUT") {
    ...
    const body = await readJson(request);
    const existingId = String(body?.id || "").trim();
    const existing = existingId ? await sb(env, "sales_quotes?id=eq." + encodeURIComponent(existingId) + "&select=*") : [];
    const beforeRow = existing?.[0] || null;
    if (beforeRow && beforeRow.deleted === true && !isSalesElevated(gate.staff)) {
  ...
  if (pathname === "/api/sales/invoices" && method === "PUT") {
    ...
    const body = await readJson(request);
    const existingId = String(body?.id || "").trim();
    const existing = existingId ? await sb(env, "sales_invoices?id=eq." + encodeURIComponent(existingId) + "&select=*") : [];
    const beforeRow = existing?.[0] || null;
    if (beforeRow && beforeRow.deleted === true && !isSalesElevated(gate.staff)) {
  ...
  if (pathname === "/api/sales/cashflow" && method === "PUT") {
    ...
    const body = await readJson(request);
    const existingId = String(body?.id || "").trim();
    const existing = existingId ? await sb(
      env,
    ...
    const createdAt = beforeRow?.created_at || body?.createdAt || (/* @__PURE__ */ new Date()).toISOString();
  ...
  if (batchRenameMatch && method === "PATCH") {
    ...
    return json({
      ...
      batch: {
        ...
        personalType: row.kind === "personal" ? normalizePersonalType(row.personal_type || "") : "",
  ...
  if (pathname === "/api/admin/overview" && method === "GET") {
    ...
    const mappedBatches = (batches || []).map((b) => ({
      ...
      personalType: b.kind === "personal" ? normalizePersonalType(b.personal_type || "") : "",
    ...
    return json({
      ...
      profiles: (profiles || []).map((p) => ({
        ...
        slugs: (cardsByProfile[p.id] || []).map((c) => ({
          ...
          personalType: c.kind === "personal" ? normalizePersonalType(c.personal_type || "business") : "",
          productId: c.product_id || "",
          status: c.status
        }))
      })),
  ...
  if (adminActivitiesMatch && method === "GET") {
    ...
    const countBy = /* @__PURE__ */ __name((key) => {
      const map = {};
  ...
  if (adminProfileMatch && method === "PUT") {
    ...
    const body = await readJson(request);
    const patch = {
      name: body.name !== void 0 ? String(body.name || "").trim() : profile.name,
      title: body.title !== void 0 ? String(body.title || "").trim() : profile.title,
      company: body.company !== void 0 ? String(body.company || "").trim() : profile.company,
      phone: body.phone !== void 0 ? String(body.phone || "").trim() : profile.phone,
      email: body.email !== void 0 ? String(body.email || "").trim().toLowerCase() : profile.email,
      whatsapp: body.whatsapp !== void 0 ? String(body.whatsapp || "").trim() : profile.whatsapp,
      linkedin: body.linkedin !== void 0 ? String(body.linkedin || "").trim() : profile.linkedin,
      youtube: body.youtube !== void 0 ? String(body.youtube || "").trim() : profile.youtube,
      x: body.x !== void 0 ? String(body.x || "").trim() : profile.x,
      instagram: body.instagram !== void 0 ? String(body.instagram || "").trim() : profile.instagram,
      tiktok: body.tiktok !== void 0 ? String(body.tiktok || "").trim() : profile.tiktok,
      website: body.website !== void 0 ? String(body.website || "").trim() : profile.website,
      address: body.address !== void 0 ? String(body.address || "").trim() : profile.address,
      menu_url: body.menuUrl !== void 0 ? String(body.menuUrl || "").trim() : profile.menu_url,
      google_review: body.googleReview !== void 0 ? String(body.googleReview || "").trim() : profile.google_review,
      check_in_url: body.checkInUrl !== void 0 ? String(body.checkInUrl || "").trim() : profile.check_in_url,
      feedback_url: body.feedbackUrl !== void 0 ? String(body.feedbackUrl || "").trim() : profile.feedback_url,
      avatar: body.avatar !== void 0 ? String(body.avatar || "").trim() : profile.avatar,
      logo: body.logo !== void 0 ? String(body.logo || "").trim() : profile.logo,
      video: body.video !== void 0 ? String(body.video || "").trim() : profile.video,
      disabled: body.disabled !== void 0 ? !!body.disabled : !!profile.disabled,
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    };
    if (Array.isArray(body.linkOrder)) {
      patch.link_order = body.linkOrder.map((k) => String(k || "").trim()).filter(Boolean).slice(0, 32);
    }
    if (body.menuPdf !== void 0) patch.menu_pdf = String(body.menuPdf || "").trim();
    if (Array.isArray(body.menuImages)) {
      patch.menu_images = body.menuImages.map((u) => String(u || "").trim()).filter(Boolean).slice(0, 20);
    }
    if (body.showPhone !== void 0) patch.show_phone = !!body.showPhone;
    if (body.showEmail !== void 0) patch.show_email = !!body.showEmail;
    if (body.showCheckin !== void 0) patch.show_checkin = !!body.showCheckin;
    if (body.showFeedback !== void 0) patch.show_feedback = !!body.showFeedback;
    if (body.showBooking !== void 0) patch.show_booking = !!body.showBooking;
    if (body.checkinForm !== void 0 && body.checkinForm && typeof body.checkinForm === "object") {
      patch.checkin_form = body.checkinForm;
    }
    if (body.feedbackForm !== void 0 && body.feedbackForm && typeof body.feedbackForm === "object") {
  ...
  if (pathname === "/api/upload" && method === "POST") {
    ...
    const kind = ["avatar", "logo", "video", "product", "menu", "catalog"].includes(kindRaw) ? kindRaw : "avatar";
    ...
    if (kind === "menu" || kind === "catalog") {
      const okType = contentType === "application/pdf" || contentType.startsWith("image/") || /\.pdf$/i.test(String(file.name || ""));
      if (!okType) {
        return bad(
    ...
    const maxBytes = kind === "product" ? 20 * 1024 * 1024 : kind === "menu" || kind === "catalog" ? 15 * 1024 * 1024 : kind === "video" ? 8 * 1024 * 1024 : kind === "avatar" ? 0 : 3 * 1024 * 1024;
    if (maxBytes > 0 && file.size > maxBytes) {
      const limitLabel = kind === "product" ? "20 MB" : kind === "menu" || kind === "catalog" ? "15 MB" : kind === "video" ? "8 MB" : "3 MB";
      const label = kind === "video" ? "Video" : kind === "menu" ? "Menu file" : kind === "catalog" ? "Catalog file" : "Image";
  ...
  if (pathname === "/api/me" && method === "PUT") {
    ...
    await sb(env, `profiles?id=eq.${encodeURIComponent(profile.id)}`, {
      method: "PATCH",
      body: {
        card_type: body.cardType !== void 0 ? body.cardType === "table" ? "table" : "personal" : profile.card_type,
        ...
        menu_pdf: body.menuPdf !== void 0 ? String(body.menuPdf || "").trim() : profile.menu_pdf || "",
        menu_images: Array.isArray(body.menuImages) ? body.menuImages.map((u) => String(u || "").trim()).filter(Boolean).slice(0, 20) : Array.isArray(profile.menu_images) ? profile.menu_images : [],
        ...
        link_order: Array.isArray(body.linkOrder) ? body.linkOrder.map((k) => String(k || "").trim()).filter(Boolean).slice(0, 32) : profile.link_order || [],
        show_phone: body.showPhone !== void 0 ? !!body.showPhone : !!profile.show_phone,
        show_email: body.showEmail !== void 0 ? !!body.showEmail : !!profile.show_email,
        show_checkin: body.showCheckin !== void 0 ? !!body.showCheckin : !!profile.show_checkin,
        show_feedback: body.showFeedback !== void 0 ? !!body.showFeedback : !!profile.show_feedback,
        show_booking: body.showBooking !== void 0 ? !!body.showBooking : profile.show_booking !== false,
        checkin_form: body.checkinForm !== void 0 && body.checkinForm && typeof body.checkinForm === "object" ? body.checkinForm : profile.checkin_form || {},
        feedback_form: body.feedbackForm !== void 0 && body.feedbackForm && typeof body.feedbackForm === "object" ? body.feedbackForm : profile.feedback_form || {},
        catalog_items: Array.isArray(body.catalogItems) ? normalizeCatalogItems(body.catalogItems) : Array.isArray(profile.catalog_items) ? profile.catalog_items : [],
  ...
  if (pathname === "/api/venue/checkins" && method === "GET") {
    ...
    if (!profile) return bad("Unauthorized", 401);
    const includeDeleted = url.searchParams.get("deleted") === "1";
    let q = `checkins?profile_id=eq.${encodeURIComponent(profile.id)}&select=*&order=created_at.desc&limit=500`;
  ...
  if (pathname === "/api/venue/checkins" && method === "POST") {
    ...
    const contact = String(body.contact || "").trim() || [phone, email].filter(Boolean).join(" \xB7 ");
  ...
  if (pathname === "/api/venue/feedback" && method === "GET") {
    ...
    if (!profile) return bad("Unauthorized", 401);
    const includeDeleted = url.searchParams.get("deleted") === "1";
    let q = `feedback?profile_id=eq.${encodeURIComponent(profile.id)}&select=*&order=created_at.desc&limit=500`;
    if (!includeDeleted) q += "&deleted=eq.false";
    const rows = await sb(env, q);
    return json({
      ok: true,
  ...
  if (catalogCartSubmitMatch && method === "POST") {
    ...
    const catalogItems = shared ? shared.catalogItems : normalizeCatalogItems(recipient.catalog_items).filter((x) => x.active !== false);
    const name = String(body?.name || "").trim().slice(0, 120);
    const email = String(body?.email || "").trim().toLowerCase().slice(0, 200);
    const phone = String(body?.phone || "").trim().slice(0, 40);
    const note = String(body?.note || "").trim().slice(0, 2e3);
    const action = String(body?.action || "quote").trim().toLowerCase();
    const status = action === "meeting" ? "meeting_booked" : action === "interest" ? "open" : "quote_requested";
    ...
    const recipientName = String(recipient.name || recipient.company || "tap-na host").trim() || "tap-na host";
    const recipientEmail = String(recipient.login_email || recipient.email || "").trim().toLowerCase();
    const money = /* @__PURE__ */ __name((n) => n === null || n === void 0 ? "Quote" : "N$ " + Number(n || 0).toLocaleString(void 0, { minimumFractionDigits: 0, maximumFractionDigits: 2 }), "money");
    const subtotal = Math.round(
      cartItems.reduce((s, l) => s + (Number(l.price) || 0) * l.qty, 0) * 100
    ) / 100;
    const linesHtml = cartItems.map(
      (l) => `<tr style="border-bottom:1px solid #eee;"><td style="padding:8px 0;">${escapeHtml(l.name)}</td><td style="padding:8px 0;">${l.qty}</td><td style="padding:8px 0;text-align:right;">${escapeHtml(money(l.price))}</td></tr>`
    ).join("");
    const linesText = cartItems.map((l) => `${l.name} \xD7 ${l.qty}${l.price != null ? ` @ ${money(l.price)}` : ""}`).join("\n");
    const quoteRef = id.replace("pcart_", "CQ-").toUpperCase();
    const subject = status === "meeting_booked" ? `Catalog meeting interest ${quoteRef} \u2014 ${name}` : `Catalog quote ${quoteRef} \u2014 ${name}`;
    try {
      const sends = [];
      sends.push(
        sendCloudflareEmail(env, {
          to: email,
          replyTo: recipientEmail.includes("@") ? recipientEmail : void 0,
          subject: status === "meeting_booked" ? `Your interest with ${recipientName}` : `Your quote request for ${recipientName}`,
          html: transactionalShell({
  ...
  if (pathname === "/api/me/catalog-carts" && method === "GET") {
    ...
    if (!profile) return bad("Unauthorized", 401);
    const includeDeleted = url.searchParams.get("deleted") === "1";
    let q = `profile_catalog_carts?profile_id=eq.${encodeURIComponent(profile.id)}&select=*&order=created_at.desc&limit=200`;
    if (!includeDeleted) q += "&deleted=eq.false";
    const rows = await sb(env, q);
  ...
  if (catalogCartPatchMatch && method === "PATCH") {
    ...
    if (body?.deleted === true) {
      patch.deleted = true;
      patch.deleted_at = (/* @__PURE__ */ new Date()).toISOString();
  ...
  if (availabilityMatch && method === "GET") {
    ...
    const fromDate = fromParam ? new Date(fromParam) : /* @__PURE__ */ new Date();
    const toDate = toParam ? new Date(toParam) : new Date(Date.now() + 60 * 24 * 60 * 60 * 1e3);
    ...
    const meetings = await sb(
      env,
      "meetings?profile_id=eq." + encodeURIComponent(profileId) + "&preferred_at=gte." + encodeURIComponent(fromIso) + "&preferred_at=lte." + encodeURIComponent(toIso) + "&status=neq.cancelled&select=id,name,email,phone,preferred_at,message,status,created_at&order=preferred_at.asc&limit=500"
    );
    const session = await getSessionProfile(env, request);
    const isOwner = !!(session && session.id === profileId);
    const taken = (meetings || []).map((m) => m.preferred_at).filter(Boolean);
    const payload = {
      ok: true,
      profileId: row.id,
  ...
  if (pathname === "/api/connections" && method === "GET") {
    ...
    const rows = await sb(
      env,
      "profile_connections?profile_id=eq." + encodeURIComponent(profile.id) + "&deleted=eq.false&order=created_at.desc&limit=500"
    );
  ...
  if (pathname === "/api/meetings" && method === "POST") {
    ...
    const conflicts = await sb(
      env,
      "meetings?profile_id=eq." + encodeURIComponent(profileId) + "&preferred_at=gte." + encodeURIComponent(windowFrom) + "&preferred_at=lte." + encodeURIComponent(windowTo) + "&status=neq.cancelled&select=id&limit=1"
    );
...
${contactText}` : "",
  ...
  if (meetingVcfMatch && method === "GET") {
    ...
    const vcf = buildGuestVcard({
      ...
      email: meeting.email || "",
      phone: meeting.phone || "",
      note: meeting.preferred_at ? `Meeting via tap-na \xB7 ${meeting.preferred_at}` : "Meeting via tap-na"
    });
  ...
  if (meetingPatchMatch && method === "PATCH") {
    ...
    await sb(env, "meetings?id=eq." + encodeURIComponent(meetingId), {
      method: "PATCH",
      body: { status, updated_at: (/* @__PURE__ */ new Date()).toISOString() },
      prefer: "return=minimal"
    });
  ...
  if (pathname === "/api/followups" && method === "POST") {
    ...
    await sb(env, "followups", {
      ...
      body: {
        ...
        note: String(body?.note || "").trim().slice(0, 4e3),
        due_at: dueAt,
  ...
  if (followupMatch && method === "PATCH") {
    ...
    const dueRaw = body?.dueAt !== void 0 ? body.dueAt : body?.due_at !== void 0 ? body.due_at : cur.due_at;
    ...
    const status = body?.status !== void 0 ? body.status === "done" ? "done" : "open" : cur.status;
    await sb(env, "followups?id=eq." + encodeURIComponent(followupId), {
      method: "PATCH",
      body: {
        meeting_id: body?.meetingId !== void 0 || body?.meeting_id !== void 0 ? String(body.meetingId || body.meeting_id || "").trim() : cur.meeting_id,
        contact_name: body?.contactName !== void 0 || body?.contact_name !== void 0 ? String(body.contactName || body.contact_name || "").trim().slice(0, 160) : cur.contact_name,
        contact_email: body?.contactEmail !== void 0 || body?.contact_email !== void 0 ? String(body.contactEmail || body.contact_email || "").trim().toLowerCase().slice(0, 160) : cur.contact_email,
        contact_phone: body?.contactPhone !== void 0 || body?.contact_phone !== void 0 ? String(body.contactPhone || body.contact_phone || "").trim().slice(0, 80) : cur.contact_phone,
        note: body?.note !== void 0 ? String(body.note || "").trim().slice(0, 4e3) : cur.note,
        due_at: dueAt,
        status,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      },
  ...
  if (pathname === "/api/shop/order-quote" && method === "POST") {
    ...
    const quoteRef = /^SQ-[A-Z0-9]+$/i.test(quoteRefRaw) ? quoteRefRaw.toUpperCase() : `SQ-${Date.now().toString(36).toUpperCase()}`;
    const salesCopyTo = "sales@tapnam.com";
    const from = defaultEmailFrom(env);
    const subject = `Quote ${quoteRef} \u2014 ${name}`;
    const issued = (/* @__PURE__ */ new Date()).toLocaleDateString("en-GB", {
      year: "numeric",
      month: "long",
      day: "numeric"
    });
    ...
    if (!validUntilLabel) {
      validUntilLabel = new Date(Date.now() + 14 * 24 * 60 * 60 * 1e3).toLocaleDateString("en-GB", {
        year: "numeric",
        month: "long",
        day: "numeric"
      });
    ...
    const imagesHtml = imageUrls.length ? `<div style="margin:0 0 16px;line-height:0;">${imageUrls.map(
      (src) => `<img src="${escapeHtml(src)}" alt="" width="72" height="72" style="display:inline-block;width:72px;height:72px;object-fit:cover;margin:0 8px 0 0;border:1px solid #eee;vertical-align:top;" />`
    ).join("")}</div>` : "";
    ...
    const buddyPayUrl = subtotal > 0 ? `https://payment.buddy.na?business=3227&amount=${encodeURIComponent(buddyAmt)}&reference=${encodeURIComponent(quoteRef)}` : "";
    const buddyHtml = buddyPayUrl ? `<p style="margin:16px 0 0;font-size:14px;"><a href="${escapeHtml(buddyPayUrl)}" style="color:#0a7;font-weight:700;">Pay with Buddy</a></p>
...
<body style="font-family:Arial,sans-serif;color:#111;line-height:1.5;max-width:560px;margin:0 auto;padding:24px;">
  ...
  ${buddyHtml}
    ...
    const text = [
      ...
      billToAddress,
      "",
      ...lines.map((l) => `${l.name} \xD7 ${l.qty} @ ${money(l.price)} = ${money(l.lineTotal)}`),
      "",
    ...
    const clientPdfName = String(clientPdf?.filename || `${quoteRef}.pdf`).trim().slice(0, 120).replace(/[^\w.\-]+/g, "_");
    const pdfAttachment = clientPdfContent.length > 100 ? {
    ...
    const productNameSummary = salesLines.length === 1 ? primary.productName : salesLines.map((l) => `${l.productName} \xD7 ${l.quantity}`).join(", ");
    ...
    if (!validUntilIso) {
      validUntilIso = new Date(Date.now() + 14 * 24 * 60 * 60 * 1e3).toISOString();
    }
    const noteParts = [
      "[Shop checkout]",
    ...
    const salesQuoteRow = {
      ...
      lines: salesLines,
      updated_at: (/* @__PURE__ */ new Date()).toISOString(),
      created_at: existingShop?.created_at || (/* @__PURE__ */ new Date()).toISOString()
    };
    try {
      await upsertSalesRow(env, "sales_quotes", salesQuoteRow);
      await writeSalesChangeLog(env, {
        staff: { id: "shop", name: "Shop checkout", email, role: "system" },
    ...
    try {
      ...
      try {
        await upsertSalesRow(env, "sales_quotes", {
          ...salesQuoteRow,
          email_status: "sent",
          emailed_at: (/* @__PURE__ */ new Date()).toISOString(),
          updated_at: (/* @__PURE__ */ new Date()).toISOString()
        });
      } catch {
      }
  ...
  if (pathname === "/api/shop/support" && method === "POST") {
    ...
    const from = defaultEmailFrom(env);
    const subject = subjectIn ? `Support: ${subjectIn} \u2014 ${name}` : `Support message from ${name}`;
    const html = `
<!DOCTYPE html>
<html>
<body style="font-family:Arial,sans-serif;color:#111;line-height:1.5;max-width:560px;margin:0 auto;padding:24px;">
  ...
  if (pathname === "/api/email/send" && method === "POST") {
    ...
    const toRaw = body?.to;
    const toList = Array.isArray(toRaw) ? toRaw.map((x) => String(x || "").trim()).filter(Boolean) : String(toRaw || "").split(",").map((x) => x.trim()).filter(Boolean);
      ...
      if (kind === "welcome") {
        ...
        sent = await sendCloudflareEmail(env, {
          ...
          html: transactionalShell({
            ...
            footerNote: (/* @__PURE__ */ new Date()).toISOString()
          }),
...
async function serveStatic(request, env) {
  ...
  assetUrl.search = "";
  const assetRequest = assetUrl.href === request.url ? request : new Request(assetUrl.toString(), request);
...
var index_default = {
  async fetch(request, env) {
    ...
    if (profileManifestMatch && request.method === "GET" && env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      try {
        ...
        const manifest = {
          ...
          description: [profile.title, profile.company].filter(Boolean).join(" \xB7 ") || `${displayName} on tap-na`,
          start_url: startUrl,
          scope: startUrl,
          display: "standalone",
    ...
    if (productTap && request.method === "GET" && isCrawler(request.headers.get("User-Agent") || "") && env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      try {
        ...
        const title = product?.name ? `${product.name} \u2014 tap-na` : "tap-na Connect card";
        const price = product && Number(product.price) > 0 ? `N$ ${Number(product.price).toLocaleString(void 0, {
          maximumFractionDigits: 2
        })}` : "";
        const descBits = [
          product?.label || "",
          price,
          String(product?.desc || "").replace(/\s+/g, " ").trim().slice(0, 140)
        ].filter(Boolean);
        const description = descBits.join(" \xB7 ") || "NFC Connect business card on tap-na. Once-off purchase. Free delivery in Windhoek.";
        const image = absolutePublicUrl(url.origin, product?.image || product?.images?.[0] || "") || shopProductFallbackImage(productId, url.origin);
        return new Response(
          ogHtml({
            title,
            description,
    ...
    if (tap && request.method === "GET" && env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY) {
      ...
      if (isCrawler(ua)) {
        try {
          ...
          const title = profile ? profile.card_type === "table" ? profile.company || profile.name || "tap-na venue" : profile.name || "Digital business card" : "tap-na card";
          const description = profile ? [profile.title, profile.company, profile.card_type === "table" ? "Venue on tap-na" : "Digital business card on tap-na"].filter(Boolean).join(" \xB7 ") : "Open this NFC / QR card on tap-na";
          const imageVersion = profile?.updated_at ? `?v=${encodeURIComponent(profile.updated_at)}` : "";
